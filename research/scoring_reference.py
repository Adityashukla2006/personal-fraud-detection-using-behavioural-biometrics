...
PROVISIONAL. This is the numpy implementation the pure-Python ``fraudcore.scoring`` must reproduce.
CLAUDE.md places all decision logic in ``src/fraudcore/`` with no third-party dependencies, so this
file exists only until that port lands and its Equal Error Rate is shown to match. Delete it then;
do not build anything new on top of it.
...
class ReferenceProfile:
    ...
    def score(self, sessions: np.ndarray) -> np.ndarray:
        ...
        if samples.shape[1] != len(self.feature_names):
            raise ValueError(
                f"sessions have {samples.shape[1]} features, "
